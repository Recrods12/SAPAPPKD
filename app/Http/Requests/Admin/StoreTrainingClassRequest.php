...
class StoreTrainingClassRequest extends FormRequest
{
    ...
    {
        return ['training_batch_id' => ['required', Rule::exists('training_batches', 'id')->whereNull('deleted_at')], 'name' => ['required', 'string', 'max:100', Rule::unique('training_classes')->where('training_batch_id', $this->integer('training_batch_id'))->whereNull('deleted_at')], 'room' => ['nullable', 'string', 'max:100'], 'capacity' => ['required', 'integer', 'min:1', 'max:500'], 'is_active' => ['required', 'boolean'], 'instructor_ids' => ['array'], 'instructor_ids.*' => ['integer', 'distinct', Rule::exists('users', 'id')->where('account_status', 'active')]];
    }
